...
import { Activity, Users, Globe, Clock, RefreshCw, TrendingUp, BarChart3, Eye, Timer } from 'lucide-react'
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-3xl font-bold">ABT Analytics</h1>
                <p className="text-blue-100">Analytics dashboard for processing {dashboardData.recordCount?.toLocaleString() || '0'} records</p>
              </div>
            </div>
            <div className="flex items-center space-x-4">
              {lastTotalRenderTime && (
                <div className="bg-white/20 px-3 py-2 rounded-lg flex items-center space-x-2">
                  <Timer className="w-4 h-4" />
                  <span className="text-sm font-medium">Total: {formatLoadingTime(lastTotalRenderTime)}</span>
                </div>
              )}
              <button
                onClick={fetchDashboardData}
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-green-600 text-sm">Complete dashboard rendering performance</p>
                </div>
              </div>
              <div className="text-right">
                <div className="text-2xl font-bold text-green-600">{formatLoadingTime(lastTotalRenderTime)}</div>
                <div className="text-sm text-green-600">Total Render Time</div>
              </div>
            </div>
            <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-white/80 rounded-lg p-3 border border-green-200">
                <div className="text-sm text-green-700 font-medium">API Response</div>
                <div className="text-lg font-bold text-gray-900">{formatLoadingTime(lastLoadingTime || 0)}</div>
              </div>
              <div className="bg-white/80 rounded-lg p-3 border border-green-200">
                <div className="text-sm text-green-700 font-medium">React Rendering</div>
                <div className="text-lg font-bold text-gray-900">
                  {formatLoadingTime((lastTotalRenderTime || 0) - (lastLoadingTime || 0))}
                </div>
              </div>
              <div className="bg-white/80 rounded-lg p-3 border border-green-200">
                <div className="text-sm text-green-700 font-medium">Efficiency</div>
                <div className="text-lg font-bold text-green-600">
                  {lastTotalRenderTime < 1000 ? '⚡ Fast' : lastTotalRenderTime < 3000 ? '🚀 Good' : '📊 Normal'}
                </div>
              </div>
            </div>
            <div className="mt-4 bg-white/80 rounded-lg p-3 border border-green-200">
              <div className="text-sm text-green-700 font-medium mb-2">Performance Breakdown</div>
              <div className="space-y-2 text-xs">
                <div className="flex justify-between">
                  <span>API Response:</span>
                  <span className="font-medium">{formatLoadingTime(lastLoadingTime || 0)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Data Processing:</span>
                  <span className="font-medium">{formatLoadingTime(50)}</span>
                </div>
                <div className="flex justify-between">
                  <span>React Rendering:</span>
                  <span className="font-medium">{formatLoadingTime((lastTotalRenderTime || 0) - (lastLoadingTime || 0) - 50)}</span>
                </div>
                <div className="flex justify-between border-t pt-1">
                  <span className="font-medium">Total Time:</span>
                  <span className="font-bold">{formatLoadingTime(lastTotalRenderTime || 0)}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              {lastTotalRenderTime && (
                <div className="text-sm text-gray-500 bg-gray-100 px-3 py-1 rounded-lg">
                  Rendered in {formatLoadingTime(lastTotalRenderTime)}
                </div>
              )}
            </div>
        ...
        <div className="mt-8 text-center">
          ...
          {lastTotalRenderTime && (
            <div className="text-xs text-gray-400 bg-gray-100 px-3 py-2 rounded-lg inline-block">
              ...
              API: {formatLoadingTime(lastLoadingTime || 0)} | 
              Rendering: {formatLoadingTime((lastTotalRenderTime || 0) - (lastLoadingTime || 0))} | 
              Efficiency: {lastTotalRenderTime < 1000 ? '⚡' : lastTotalRenderTime < 3000 ? '🚀' : '📊'}
            </div>
          )}
        </div>
      </div>
